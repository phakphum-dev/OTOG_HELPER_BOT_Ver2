    ...
    while True:
            ...
            if not dataBASS.contest["ann"]:
                ...
                if state == -1:
                    if (
                        ...
                        <= abs(
                            time.time()
                            - ContestManager.timeParse(
                                dataBASS.contest["info"]["timeStart"]
                            )
                        )
                        <= 61 * 60
                    ):
        ...
        if util.isSleepTime():
            ...
            if userLife == -1:
                botStatus = Status.dnd
                botActivi = Activity(
                    name="ไม่รู้ว่ามีคนทำโจทย์กี่คน... help()",
                    type=ActivityType.playing,
                    url="https://otog.cf/",
                )
            elif userLife == 0:
            ...
            botName = (
                f"{dataBASS.contest['info']['id']}:{dataBASS.contest['info']['name']}"
            )
            botStatus = Status.dnd
            botActivi = Game(
...
async def sayhelp(chan, isAdmin: bool = False):
    em = Embed(
        title=":grey_question:สิ่งที่น้อมทำได้:grey_question:",
        description="มีแค่นี้แหละ",
        colour=Colour.from_rgb(255, 133, 29),
    )
    em.add_field(
        name=":grey_question:help()", value="ก็ที่ทำอยู่ตอนนี้แหละ", inline=False
    )
    em.add_field(name=":trophy:contest()", value="คอนเทสที่กำลังจะมาถึง", inline=False)
    em.add_field(
        name=":person_playing_handball:tasks()", value="จำนวนโจทย์ตอนนี้", inline=False
    )
    em.add_field(
        name=":military_medal:ranking()", value="คำสั่งไว้ขิงกัน", inline=False
    )
    em.add_field(
        name=":question:question(<ชื่อโจทย์>) <คำถาม>",
        value="ถามคำถามเกี่ยวกับโจทย์ <ชื่อโจทย์>\nและ<คำถาม>ควรตอบเป็น Yes/No(ใช่/ไม่ใช่)",
        inline=False,
    )
    em.add_field(
        name=":wrench:change_Log()",
    ...
    if isAdmin:
        em = Embed(
            title=":grey_question:สิ่งที่แอดมินทำได้:grey_question:",
            description="แค่ในนี้เท่านั้น",
            colour=Colour.red(),
        )
        ...
        em.add_field(
            name=":loudspeaker:say(<Channel_ID>) <Text>",
            value="ส่ง <Text> ไปยังห้อง <Channel_ID>",
            inline=False,
        )
        em.add_field(
            name=":eyes:read() <Text>",
            value="เป็นการสั่งให้ตัว Console อ่าน <Text> แล้วทำการปริ้นออกมา",
            inline=False,
        )
        em.add_field(
            name=":question:q_answer(<id>) <text>",
            value="ตอบคำถามที่ <id> โดยคำถามจะหายด้วย",
            inline=False,
        )
        em.add_field(
            name=":question:q_remove(<id>)", value="ลบคำถามที่ <id>", inline=False
        )
        em.add_field(
            name=":question:q_clear()",
            value="clear คำถามทั้งหมด(ต้องแน่ใจจริงๆว่าจะทำ)",
            inline=False,
        )
        em.add_field(
            name=":exclamation:test()", value="ดูว่าน้องยังมีชีวิตอยู่ไหม", inline=False
        )
        em.add_field(
            name=":exclamation:test_Verify()\\n<Code in C/C++>",
            value="ทดสอบว่า Grader แมวๆยังใช้ได้ไหม",
            inline=False,
        )
        em.add_field(
            name=":exclamation:check_Verify()",
            value="ดูว่ามีใครมา Verify ไหม",
            inline=False,
        )
        em.add_field(
            name=":exclamation:Watch_Code_Verify(<id>)",
            value="ดู Code ของ <id>",
            inline=False,
        )
        em.add_field(
            name=":exclamation:Force_Reload()",
            value="บังคับให้รีโหลดฐานข้อมูลใหม่",
            inline=False,
        )
        em.add_field(
            name=":sleeping_accommodation:shutdown()",
            value="ชื่อก็บอกอยู่แล้ว",
            inline=False,
        )
        em.add_field(
            name=":checkered_flag:Score_Board(<id>)",
            value="ไว้ใช้ดูคะแนนในคอนเทส <id>",
            inline=False,
        )
        em.add_field(
            name=":checkered_flag:Send_Score_Board(<id>, <channel_id>)",
            value="ไวส่งคะแนนใน <channel_id> ของคอนเทส <id>",
            inline=False,
        )
        await chan.send(content=None, embed=em)
...
async def sayRanking(mes):
    ...
    if ranking == -1:
        ...
        thisEm = Embed(
            title="Ranking <:oo:706363373079756851><:t_:706363391366660107><:og:706363407124922389><:g_:706363356172255263>",
            colour=Colour.from_rgb(255, 133, 29),
        )
        ...
        for i in range(0, min(10, len(ranking))):
            thisEm.add_field(
                name=util.nameWithPlace(ranking[i][1], i + 1), value=str(ranking[i][0])
            )
...
async def sayContestRanking(conId: int, chan):
    ...
    if data == -1:
        ...
        thisEm = Embed(
            title=f"Scoreboard Contest#{conId}",
            colour=Colour.from_rgb(255, 133, 29),
            url="https://otog.cf/contest/history/" + str(conId),
        )
        thisEm.set_author(
            name=f"{data['name']}", icon_url="https://otog.cf/logo196.png"
        )

        for i in range(0, min(10, len(data["ranking"]))):
            thisName = util.nameWithPlace(
                f"{data['ranking'][i][0]} ({sum(data['ranking'][i][1])}) ใช้เวลา {data['ranking'][i][2]/1000:.2f} วินาที",
                i + 1,
            )
...
async def sayContestInfo(mes):
    ...
    if "info" not in dataBASS.contest:
        ...
        thisEm = Embed(
            title=f"Incoming Contest#{conInfo['id']}",
            colour=Colour.from_rgb(255, 133, 29),
        ...
        thisEm.add_field(
            name="เวลาเริ่ม",
            value=util.formatInThai(ContestManager.timeParse(conInfo["timeStart"])),
            inline=False,
        )
        thisEm.add_field(
            name="เวลาจบ",
            value=util.formatInThai(ContestManager.timeParse(conInfo["timeEnd"])),
            inline=False,
        )
        thisEm.add_field(
            name="ระยะเวลา",
            value=util.lenTimeInThai(
                ContestManager.timeParse(conInfo["timeStart"]),
                ContestManager.timeParse(conInfo["timeEnd"]),
            ),
            inline=False,
        )
...
async def gngHowto(chan):
    ...
    thisEM.add_field(
        name=":1234:วิธีการเล่นคือ",
        value="ข้าจะคิดเลขหนึ่งตัวตั้งแต่ 1 ถึง 100 เจ้าต้องทายเลขของข้าให้ถูกภายใน 7 ครั้ง",
        inline=False,
    )
    thisEM.add_field(
        name="สามารถทายโดยการ guess(ตัวเลข) เช่น guess(12)",
        value="นั้นคือเจ้าทายว่าเลขของข้านั้นคือ 12",
        inline=False,
    )
    thisEM.add_field(
        name=":arrow_down:ถ้าเลขที่เจ้าตอบมันต่ำกว่า",
        value="ข้าก็จะบอกต่ำไป",
        inline=False,
    )
    thisEM.add_field(
        name=":arrow_up:แต่ถ้าเลขเจ้ามันสูงไป", value="ข้าก็จะบอกสูงไป", inline=False
    )
    thisEM.add_field(
        name=":white_check_mark:แต่ถ้าถูก", value="ข้าจะบอกว่าถูกเอง", inline=False
    )
    thisEM.add_field(
        name=":x:ถ้าเจ้ากลัวที่จะแพ้ข้า",
        value="ก็สามารถออกได้โดยการ guess(*) เอา หึๆๆๆ",
        inline=False,
    )
    await chan.send(content=None, embed=thisEM)
